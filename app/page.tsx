...
export default function Page() {
  ...
  function scrollTo(id: string) {
    setMenuOpen(false)
    // An explicit behavior option overrides the computed scroll-behavior, so the
    // `html { scroll-behavior: auto }` reduced-motion reset in globals.css cannot
    // reach this call. Ask for the preference here instead.
    const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches
    document.getElementById(id)?.scrollIntoView({ behavior: reduced ? 'auto' : 'smooth' })
  }

  // header/footer only expose the banner and contentinfo landmarks while they sit
  // outside <main>; nested inside it they collapse to generic. A plain <div> is not
  // a sectioning element, so it suppresses neither role -- and keeping a single
  // element as the sole body child preserves the stacking context that
  // `body > * { z-index: 1 }` in globals.css establishes, which the hero's text
  // antialiasing turns out to be sensitive to.
  return (
    <div className="min-h-screen overflow-hidden text-foreground">
      <header id="top" className="hero-sheet blueprint-surface blueprint-invert relative w-full overflow-hidden bg-primary text-primary-foreground">
        <a href="#main" className="hero-copy absolute left-5 top-5 -translate-y-[200%] bg-accent px-4 py-3 text-xs font-bold uppercase tracking-[0.12em] text-accent-foreground focus:translate-y-0">Skip to main content</a>
        ...
        <nav className="hero-copy relative mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-5 md:px-8 lg:px-10" aria-label="Main navigation">
          ...
          <button className="-m-2.5 p-2.5 md:hidden" onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? 'Close menu' : 'Open menu'} aria-expanded={menuOpen} aria-controls="mobile-menu">{menuOpen ? <X /> : <Menu />}</button>
        </nav>
        {menuOpen && <div id="mobile-menu" className="hero-copy relative mx-auto flex w-full max-w-7xl flex-col gap-5 border-y border-primary-foreground/30 px-5 py-5 text-sm font-semibold uppercase tracking-[0.12em] md:hidden"><button onClick={() => scrollTo('programs')} className="text-left">What we do</button><button onClick={() => scrollTo('story')} className="text-left">Our story</button><button onClick={() => scrollTo('contact')} className="text-left">Partner with us</button></div>}
      ...
      <main id="main">
        ...
        <section className="border-y border-border bg-muted/40" id="programs"><div className="mx-auto max-w-7xl px-5 py-16 md:px-8 md:py-24 lg:px-10"><div className="mb-10 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="eyebrow">The blueprint</p><h2 className="mt-4 text-4xl font-black uppercase tracking-[-0.05em] md:text-6xl">Four ways in.</h2></div><p className="max-w-xs text-sm leading-6 text-muted-foreground">Different entry points. One shared ambition: build businesses that matter.</p></div><div className="grid gap-px border border-border bg-border md:grid-cols-2">{programs.map((program) => <button key={program.id} onClick={() => setActiveProgram(program.id)} className={`group relative min-h-64 overflow-hidden bg-background p-6 text-left transition-colors md:p-8 ${activeProgram === program.id ? 'blueprint-invert bg-primary text-primary-foreground' : 'hover:bg-accent/20'}`}>{activeProgram === program.id && <div className="blueprint-layer blueprint-grid" aria-hidden="true" />}<div className="relative flex items-start justify-between"><span className="font-mono text-xs">{program.number}</span><ArrowUpRight className={`size-5 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1 ${activeProgram === program.id ? 'text-accent' : ''}`} /></div><div className="relative mt-16"><p className="mb-2 text-xs font-bold uppercase tracking-[0.16em] text-accent">{program.label}</p><h3 className="text-2xl font-black uppercase tracking-[-0.03em]">{program.title}</h3></div></button>)}</div><div className="blueprint-surface blueprint-invert grid gap-8 border-x border-b border-border bg-primary p-7 text-primary-foreground md:grid-cols-[1fr_2fr] md:p-10"><div className="blueprint-layer blueprint-grid" aria-hidden="true" /><p className="relative font-mono text-xs uppercase tracking-[0.15em] text-primary-foreground/60">{active.label} / 0{programs.indexOf(active) + 1}</p><div className="relative"><p className="max-w-xl text-xl leading-8">{active.copy}</p><button onClick={() => scrollTo('contact')} className="mt-6 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-accent">Get involved <ArrowUpRight className="size-4" /></button></div></div></div></section>
        ...
        <section className="blueprint-surface blueprint-invert relative mx-3 overflow-hidden bg-primary px-6 py-16 text-primary-foreground md:mx-5 md:px-12 md:py-20 lg:mx-8 lg:px-16" id="watch">
          <div className="blueprint-layer blueprint-grid" aria-hidden="true" />
          <div className="relative grid items-center gap-10 md:grid-cols-[1fr_auto] md:gap-16">
            <div>
              ...
              <h2 className="mt-5 max-w-lg text-balance text-5xl font-black uppercase leading-[0.9] tracking-[-0.06em] md:text-6xl">See it for yourself.</h2>
              <p className="mt-8 max-w-md text-lg leading-7 text-primary-foreground/80">Three minutes inside the room — the conversations, the people, and the energy that make this more than another business event.</p>
              <p className="mt-8 flex items-center gap-3 font-mono text-xs uppercase tracking-[0.2em] text-primary-foreground/50"><Play className="size-4 text-accent" /> 2:58 / Kampala</p>
            </div>
